...
<div class="flex items-center bg-neutral-50 px-3 py-5 rounded-xl gap-3">
    ...
    <div class="flex justify-center shrink-0">
        <x-client.select variant="select" />
    </div>
    ...
    <div class="flex flex-1 gap-3 md:hidden items-center">
        ...
        <div class="flex-1 min-w-0 flex flex-col gap-3">
            <div class="flex flex-col">
                ...
                <p class="text-xs text-neutral-400 mb-0.5">Varian</p>
                <p class="text-sm text-neutral-500 line-through">Rp{{ number_format($product->selling_price, 0, ',', '.') }}</p>
                <p class="text-[16px] font-semibold text-primary-500">Rp{{ number_format($product->selling_price - ($product->selling_price * $product->discount / 100), 0, ',', '.') }}</p>
    ...
    <div class="hidden md:grid grid-cols-[2fr_1fr_1fr_1fr_2rem] flex-1 items-center">
        ...
        <p class="text-sm text-neutral-500 text-center">—</p>
        <div class="text-center">
            <p class="text-[10px] text-neutral-400">-</p>
            <p class="text-sm text-neutral-500 line-through">Rp{{ number_format($product->selling_price, 0, ',', '.') }}</p>
            <p class="text-lg font-semibold text-primary-500">Rp{{ number_format($product->selling_price - ($product->selling_price * $product->discount / 100), 0, ',', '.') }}</p>
        </div>
