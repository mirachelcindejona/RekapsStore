            ...
            <div class="flex flex-col gap-[20px]">
                <div>
                    ...
                    <div class="flex items-center gap-[16px] flex-wrap">
                        <div class="flex gap-[8px]">
                            ...
                            <span
                                class="px-[12px] py-[4px] rounded-full text-[10px] font-bold uppercase bg-[#f0fdf4] text-[#16a34a]">{{ $product->status }}</span>
                        </div>
                    </div>
                ...
                <div class="flex flex-col gap-[4px]">
                    ...
                    <div class="text-[14px] font-medium text-neutral-500 mt-[4px]">
                        Modal : Rp {{ number_format($product->cost_price, 0, ',', '.') }} &nbsp;|&nbsp; Margin : Rp
                        {{ number_format($margin, 0, ',', '.') }}
                    </div>

                    <div class="text-[14px] font-medium text-neutral-500 mt-[4px]">
                        Info Pengambilan: <span
                            class="text-neutral-700 font-semibold">{{ $product->pickup_info ?: 'Belum ada informasi' }}</span>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap gap-y-[24px] gap-x-[40px] py-[16px] border-y border-neutral-200">
                    <div class="flex flex-col gap-[4px]">
                        ...
                        <span class="text-[20px] font-bold text-primary-500">{{ $totalStock }}</span>
                    </div>
                    <div class="flex flex-col gap-[4px]">
                    ...
                    <div class="flex flex-col gap-[4px]">
                        ...
                        <span class="text-[20px] font-bold text-neutral-950">{{ $product->product_code }}</span>
                    </div>
                </div>
                        ...
                        <div class="flex flex-wrap gap-[8px]">
                            @foreach ($product->variants as $var)
                                <div
                                    ...
                                    {{ $var->variant_value }} <span
                                        class="font-normal opacity-80 text-[12px] ml-1">(Online: {{ $var->stock_online }} |
                                        Bazar: {{ $var->stock_bazar }})</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
                        ...
                        <div class="flex flex-row gap-[16px] w-full flex-1">
                            <div
                                ...
                                <span class="text-[12px] font-semibold text-neutral-500 uppercase relative z-10">Stok
                                    Online</span>
                                <span
                                    class="text-[36px] font-bold text-neutral-950 leading-none relative z-10">{{ $onlineStock }}</span>
                            </div>
                            ...
                            <div
                                ...
                                <span class="text-[12px] font-semibold text-neutral-500 uppercase relative z-10">Stok
                                    Bazar</span>
                                <span
                                    class="text-[36px] font-bold text-neutral-950 leading-none relative z-10">{{ $bazarStock }}</span>
                            </div>
                        </div>
        ...
        <div class="mt-[32px]">
            ...
            <div class="p-0 min-[768px]">
                ...
                <div id="tab-ulasan" class="tab-content hidden animate-[fadeIn_0.4s_ease]">
                    <div class="flex flex-col md:flex-row gap-[20px] mb-[24px]">
                        <div
                            ...
                            <div class="text-[48px] font-bold text-primary-500 leading-none">{{ $avgRating }}</div>
                            <div class="flex gap-[2px] text-[#ffdf20] text-[16px]">
                                @for ($i = 1; $i <= 5; $i++)
                                    {{ $i <= round($avgRating) ? '★' : '☆' }}
                                @endfor
                            </div>
                            <div class="text-[14px] font-medium text-neutral-400">{{ $totalReviews }} Ulasan</div>
                        </div>
                        ...
                        <div
                            ...
                            @foreach ([5, 4, 3, 2, 1] as $star)
                                <div class="flex items-center gap-[8px]">
                                    ...
                                    <div class="w-[15px] text-[12px] text-neutral-400 text-right">{{ $starCounts[$star] }}
                                    </div>
                                </div>
                            @endforeach
                        </div>
                        ...
                        @forelse($reviews as $review)
                            <div
                                ...
                                <div class="flex items-center gap-[10px]">
                                    ...
                                    @endphp
                                    <div
                                        class="w-[36px] h-[36px] rounded-full flex items-center justify-center text-neutral-50 text-[12px] font-bold {{ $color }}">
                                        {{ $initials }}
                                    </div>
                                    <div>
                                        <div class="text-[14px] font-bold text-neutral-950">
                                            {{ $review->user->name ?? 'Anonim' }}</div>
                ...
                <div id="tab-stok" class="tab-content hidden animate-[fadeIn_0.4s_ease]">
                    ...
                    <div class="flex flex-col">
                        @forelse($product->stockHistories()->latest()->get() as $history)
                            <div
                                ...
                                <div class="flex items-center gap-[16px]">
                                    ...
                                    <div>
                                        <div class="text-[14px] font-bold text-neutral-950 mb-[2px]">
                                            ...
                                            {{ $history->note ?? 'Perubahan Stok' }}
                                        </div>
                                        <div class="text-[12px] text-neutral-400">
                                            Oleh: {{ $history->user->name ?? 'Sistem' }} | Lokasi:
                                            {{ $history->location }} | {{ $history->created_at->format('d M Y, H:i') }}
                                        </div>
                                    </div>
                                </div>
...
@section('footer')
                ...
                @if ($hasVariants)
                    <div class="flex flex-col gap-[8px]">
                        ...
                        <select name="variant_id" required
                            class="bg-neutral-50 border border-neutral-500 rounded-[10px] p-[12px_15px] text-[14px] text-neutral-700 w-full outline-none transition-all duration-200 focus:border-primary-500 focus:shadow-[0_0_0_3px_rgba(125,57,235,0.15)] appearance-none">
                            @foreach ($product->variants as $var)
                                <option value="{{ $var->id }}">{{ $var->variant_value }}</option>
                            @endforeach
                        </select>
                    </div>
                @endif
    ...
    <div id="modalPindahStok"
        ...
        <div
            ...
            <form action="{{ url('/admin/product/' . $product->slug . '/stock-transfer') }}" method="POST"
                class="flex flex-col gap-[16px]" onsubmit="disableSubmitButton(this)">
                @csrf
    ...
    <div id="modalConfirmDelete"
        ...
        <div
            ...
            <form action="{{ url('/admin/product/' . $product->slug) }}" method="POST"
