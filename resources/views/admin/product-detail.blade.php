...
@section('page_breadcrumb', 'Detail Produk')
    ...
    <div
        ...
        <div class="grid grid-cols-1 min-[900px]:grid-cols-[350px_1fr] gap-[32px] items-start">
            ...
            <div class="flex flex-col gap-[12px] w-full">
                @php $mainImage = $product->images->first() ? asset('storage/'.$product->images->first()->image_path) : asset('assets/img/default-product.png'); @endphp
                <img id="main-image-display" src="{{ $mainImage }}" alt="{{ $product->name }}"
            ...
            <div class="flex flex-col gap-[20px]">
                <div>
                    <h1 class="text-[24px] font-bold text-neutral-950 m-0 mb-[8px]">{{ $product->name }}</h1>

                    <div class="flex items-center gap-[8px] flex-wrap mb-[10px]">
                        <span
                            class="px-[12px] py-[4px] rounded-full text-[10px] font-bold uppercase bg-primary-100 text-primary-500">{{ $product->product_type }}</span>
                        <span
                            class="px-[12px] py-[4px] rounded-full text-[10px] font-bold uppercase bg-[#fefce8] text-[#d08700]">{{ $product->category->name ?? 'Kategori' }}</span>
                        <span
                            class="px-[12px] py-[4px] rounded-full text-[10px] font-bold uppercase {{ $product->status == 'Aktif' ? 'bg-[#f0fdf4] text-[#16a34a]' : 'bg-neutral-200 text-neutral-500' }}">{{ $product->status }}</span>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-[8px] bg-neutral-100 p-[16px] rounded-xl border border-neutral-200">
                    ...
                    <div class="text-[13px] font-medium text-neutral-500">
                        ...
                        Modal: <strong>Rp {{ number_format($product->cost_price, 0, ',', '.') }}</strong> &nbsp;|&nbsp;
                        Margin: <strong class="text-[#8DB524]">Rp {{ number_format($margin, 0, ',', '.') }}</strong>
                    </div>
                    <div
                        ...
                        <span><strong>Info Pengambilan:</strong>
                            {{ $product->pickup_info ?: 'Tidak ada instruksi khusus.' }}</span>
                    </div>
                </div>
                            ...
                            <div class="flex justify-between items-start pl-[8px]">
                                <div class="flex flex-col">
                                    <span class="text-[12px] font-bold text-neutral-500 uppercase tracking-wider">Total Stok
                                        Online</span>
                                    <span
                                        class="text-[32px] font-extrabold text-neutral-900 leading-none mt-[4px]">{{ $product->variants->sum('stock_online') ?? ($product->inventory->main_stock ?? 0) }}</span>
                                </div>
                                ...
                                <div
                                    ...
                                    @foreach ($product->variants as $variant)
                                        <div
                                            ...
                                            <span class="text-neutral-600 font-medium">{{ $variant->variant_name }}</span>
                                            <span
                                                class="font-bold text-neutral-900 bg-neutral-100 px-[8px] py-[2px] rounded-md">{{ $variant->stock_online ?? 0 }}</span>
                                        </div>
                                    @endforeach
                                </div>
                            @endif
                            ...
                            <div class="flex justify-between items-start pl-[8px]">
                                <div class="flex flex-col">
                                    <span class="text-[12px] font-bold text-neutral-500 uppercase tracking-wider">Total Stok
                                        Bazar</span>
                                    <span
                                        class="text-[32px] font-extrabold text-neutral-900 leading-none mt-[4px]">{{ $product->variants->sum('stock_bazar') ?? ($product->inventory->bazar_stock ?? 0) }}</span>
                                </div>
                                ...
                                <div
                                    ...
                                    @foreach ($product->variants as $variant)
                                        <div
                                            ...
                                            <span class="text-neutral-600 font-medium">{{ $variant->variant_name }}</span>
                                            <span
                                                class="font-bold text-neutral-900 bg-neutral-100 px-[8px] py-[2px] rounded-md">{{ $variant->stock_bazar ?? 0 }}</span>
                                        </div>
                                    @endforeach
                                </div>
                            @endif
            ...
            <div class="p-0 min-[768px]:p-[16px]">
                ...
                <div id="tab-deskripsi" class="tab-content block animate-[fadeIn_0.4s_ease]">
                    <div class="text-[14px] text-neutral-700 leading-relaxed whitespace-pre-wrap">
                        {{ $product->description ?: 'Belum ada deskripsi.' }}</div>
                </div>

                <div id="tab-stok" class="tab-content hidden animate-[fadeIn_0.4s_ease]">
                    <div class="flex flex-col">
                        @forelse($product->stockHistories()->with('user', 'variant')->latest()->get() as $history)
                            <div
                                ...
                                <div class="flex items-center gap-[16px]">
                                    <div
                                        class="w-[36px] h-[36px] rounded-full flex items-center justify-center shrink-0 border-2 {{ $history->type == 'Masuk' ? 'border-[#00c951] text-[#00c951]' : 'border-[#fb2c36] text-[#fb2c36]' }}">
                                    ...
                                    <div>
                                        <div class="text-[14px] font-bold text-neutral-950 mb-[2px]">
                                            {{ $history->type == 'Masuk' ? '+' : '-' }}{{ $history->qty }} pcs -
                                            {{ $history->note }}
                                        </div>
                                        <div class="text-[12px] text-neutral-500">
                                            @if ($history->variant)
                                                <span
                                                    class="font-bold text-primary-500">[{{ $history->variant->name }}]</span>
                                                •
                                            @endif
                                            Lokasi: {{ $history->location }} •

                                            Oleh: <span
                                                class="text-neutral-800 font-semibold">{{ $history->user->name ?? 'System/Deleted' }}</span>
                                            •
                                ...
                                <div class="text-left md:text-right ml-[52px] md:ml-0">
                                    <div
                                        class="text-[14px] font-bold {{ $history->type == 'Masuk' ? 'text-[#00c951]' : 'text-[#fb2c36]' }}">
                                        {{ $history->type == 'Masuk' ? '+' : '-' }}{{ $history->qty }}
                                    </div>
                                </div>
                            </div>
                        @empty
...
@section('footer')
    <div id="modalStokMasuk"
        ...
        <form action="{{ url('/admin/product/' . $product->slug . '/stock') }}" method="POST"
    ...
    <div id="modalStokKeluar"
        class="modal fixed inset-0 bg-black/45 hidden items-center justify-center z-[9999] opacity-0 transition-all duration-300 p-[16px] [&.active]:flex [&.active]:opacity-100">
        <form action="{{ url('/admin/product/' . $product->id . '/stock') }}" method="POST"
            class="w-full max-w-[500px] bg-white shadow-lg rounded-[20px] p-[24px_20px] sm:p-[32px] flex flex-col gap-[20px] transform translate-y-5 transition-transform duration-300 max-h-[90vh] overflow-y-auto [&.active_.modal-container]:translate-y-0">
            @csrf
