...
<div id="logoutModal" class="hidden fixed inset-0 z-[9999] flex items-center justify-center bg-black/40">
    <div class="bg-white w-[360px] rounded-2xl p-8 shadow-xl">
        ...
        <p class="text-sm text-neutral-500 mb-6">Kamu yakin ingin logout dari panel admin?</p>
        <div class="flex justify-end gap-3">
            ...
            <button onclick="document.getElementById('logoutFormAdmin').submit()"
...
<script>
    ...
    {{-- Sidebar Toggle — SELALU ADA, tidak boleh di dalam @if --}}
    <script>
...
{{-- Toast sukses --}}
@if(session('success'))
<div id="toast-success" class="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-green-600 text-white px-6 py-4 rounded-xl shadow-lg">
    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
    </svg>
    <span class="text-sm font-medium">{{ session('success') }}</span>
    <button onclick="document.getElementById('toast-success').remove()" class="ml-2 text-white/80 hover:text-white text-lg">&times;</button>
</div>
<script>
    setTimeout(() => {
