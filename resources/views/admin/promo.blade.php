...
@section('title', 'Diskon dan Voucher')
@section('page_title', 'Diskon dan Voucher')
    ...
    <div class="bg-white rounded-[24px] p-6 shadow-sm border border-gray-100 mb-6">
        ...
        <div class="flex justify-between items-center mb-5">
            ...
            <button onclick="openDiskon()"
                class="bg-primary-500 text-white px-4 py-2 rounded-xl text-sm font-semibold hover:opacity-90">
                + Buat Diskon
            </button>
        </div>

        <div class="overflow-x-auto touch-pan-x custom-scrollbar pb-[10px]">
            <table
                        ...
                        <tr class="hover:bg-primary-50 transition-colors duration-[250ms]">
                            ...
                            <td
                                ...
                                <span class="bg-emerald-100 text-emerald-500 px-3 py-1 rounded-full text-xs">
                                    {{ $discount->status == 'active' ? 'Aktif' : 'Non-Aktif' }}
                                </span>
                            </td>
                            ...
                            <td
                                ...
                                <div class="flex gap-[6px]">
                                    ...
                                    {{-- DELETE --}}
                                    <form id="deleteFormDiscount{{ $discount->id }}"
                                        action="{{ route('admin.discount.destroy', $discount->id) }}" method="POST">

                                        @csrf
                                        @method('DELETE')

                                        <button type="button" onclick="openDeleteDiscountModal({{ $discount->id }})"
                                            class="flex items-center justify-center w-[36px] h-[36px]
...
@section('footer')
            ...
            <form action="{{ route('admin.voucher.store') }}" method="POST">
                ...
                <div class="form-grid grid grid-cols-1 md:grid-cols-2 gap-[20px]">
                    ...
                    <div class="form-group flex flex-col gap-1.5">
                        ...
                        <input type="number" name="min_purchase" min="0" required
                            class="p-[10px] rounded-[10px] border border-gray-200 outline-none focus:border-primary-500 transition-all">
                    </div>
            ...
            <form action="{{ route('admin.discount.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-[20px]">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="text" name="name" required placeholder="CTH : Jersey"
                            class="p-[10px] rounded-[10px] border border-neutral-200 outline-none focus:border-primary-500">
                    </div>
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="number" name="value" min="0" required placeholder="10"
                            class="p-[10px] rounded-[10px] border border-neutral-200 outline-none focus:border-primary-500">
                    </div>
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="number" name="min_purchase" min="0" required placeholder="50000"
                            class="p-[10px] rounded-[10px] border border-neutral-200 outline-none focus:border-primary-500">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="number" name="quota" min="1" required placeholder="50"
                            class="p-[10px] rounded-[10px] border border-neutral-200 outline-none focus:border-primary-500">
                    </div>
                ...
                <div class="modal-footer flex justify-end gap-[12px] mt-[24px]">
                    ...
                    <button type="button" class="bg-gray-100 px-[20px] py-[10px] rounded-[12px] font-medium"
                        onclick="closeAll()">
                        Batal
                    </button>

                    <button type="submit"
                        class="bg-secondary-500 text-gray-900 px-[20px] py-[10px] rounded-[12px] font-bold">
                        Simpan
                    </button>
                </div>
            </form>
    ...
    {{-- edit diskon --}}
    <div id="editDiscountModal" class="fixed inset-0 z-[100] hidden flex items-center justify-center p-4">
        <div class="bg-white w-[90%] max-w-[600px] rounded-[20px] p-[24px] shadow-xl">
            <div class="flex justify-between items-center mb-5">
                <h2 class="text-lg font-bold">
                    Edit Diskon
                </h2>

                <span onclick="closeEditDiscount()" class="cursor-pointer text-xl">
                    ✕
                </span>
            </div>
            ...
            <form id="editDiscountForm" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input id="edit_discount_name" name="name" type="text"
                            class="w-full border rounded-xl p-2 border-gray-200 outline-none focus:border-primary-500 transition-all">
                    </div>

                    <div>
                        <label>Tipe Diskon</label>
                        <select id="edit_discount_type" name="type" class="w-full border rounded-xl p-2">
                            <option value="fixed">Nominal (Rp)</option>
                            <option value="percentage">Persentase (%)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input id="edit_discount_value" type="number" name="value" min="0" required
                            class="w-full border rounded-xl p-2 border-gray-200 outline-none focus:border-primary-500 transition-all">
                    </div>

                    <div>
                        <label>Min Pembelian</label>
                        <input id="edit_discount_min_purchase" type="number" name="min_purchase" min="0"
                            required
                            class="w-full border rounded-xl p-2 border-gray-200 outline-none focus:border-primary-500 transition-all">
                    </div>
                    ...
                    <div>
                        <label>Kuota</label>
                        <input id="edit_discount_quota" type="number" name="quota" min="1" required
                            class="w-full border rounded-xl p-2 border-gray-200 outline-none focus:border-primary-500 transition-all">
                    </div>

                    <div>
                        <label>Berlaku Hingga</label>
                        <input id="edit_discount_end_date" type="date" name="end_date"
                    ...
                    <div>
                        <label>Status</label>
                        <select id="edit_discount_status" name="status"
                            class="w-full border rounded-xl p-2 border-gray-200 outline-none focus:border-primary-500 transition-all">
                            <option value="active">
                                Aktif
                            </option>
                ...
                <div class="flex justify-end gap-3 mt-5">
                    <button type="button" onclick="closeEditDiscount()" class="px-4 py-2 bg-neutral-100 rounded-xl">
                        Batal
                    </button>
    ...
    <div id="deleteDiscountModal" class="fixed inset-0 z-[9999] hidden items-center justify-center bg-black/30">
        <div class="w-[420px] rounded-[24px] border border-black bg-white p-[32px] shadow-xl">
            ...
            <div class="flex justify-end gap-3 mt-8">
                <button type="button" onclick="closeDeleteDiscountModal()" class="px-5 py-2 rounded-xl bg-neutral-200">
                    Batal
                </button>
                <button type="button" onclick="submitDeleteDiscount()"
                    class="px-5 py-2 rounded-xl bg-red-500 text-neutral-50">
                    Hapus
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        function submitDelete() {
            document
                .getElementById('deleteForm' + deleteVoucherId)
                .submit();
        }

        let deleteDiscountId = null;

        function openDeleteDiscountModal(id) {
            deleteDiscountId = id;

            document
                .getElementById('deleteDiscountModal')
                .classList.remove('hidden');
        ...
        function submitDeleteDiscount() {
            document
                .getElementById(
                    'deleteFormDiscount' + deleteDiscountId
                )
                .submit();
        }
        ...
        function editDiscount(
            ...
            document
                .getElementById('edit_discount_type')
                .value = type;
            ...
            document
                .getElementById('edit_discount_min_purchase')
                .value = minPurchase;

            document
                .getElementById('edit_discount_quota')
                .value = quota;

            document
                .getElementById('edit_discount_end_date')
                .value = endDate;

            document
                .getElementById('edit_discount_status')
                .value = status;
